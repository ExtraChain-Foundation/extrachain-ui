...
object DefaultNetworkListener {
    ...
    suspend fun get() = if (fallback && Build.VERSION.SDK_INT >= 23) @TargetApi(23) {
    ...
    private fun register() {
        val conn = connectivity ?: return

        when (Build.VERSION.SDK_INT) {
            in 31..Int.MAX_VALUE -> @TargetApi(31) {
                conn.registerBestMatchingNetworkCallback(
                    request,
                    Callback,
                    mainHandler
                )
            ...
            in 28 until 31 -> @TargetApi(28) {  // we want REQUEST here instead of LISTEN
                conn.requestNetwork(request, Callback, mainHandler)
            }

            in 26 until 28 -> @TargetApi(26) {
                conn.registerDefaultNetworkCallback(Callback, mainHandler)
            }
