...
ConsoleCommandProcessor::ConsoleCommandProcessor(ExtraChainNode& _node, QObject* parent)
    : node(_node)
    , QObject(parent) {
}
...
void ConsoleCommandProcessor::showWalletsList(const std::string& input) {
    ...
    if (balances.empty()) {
        ...
        // const auto&        wallets_names = node.accountController()->currentProfile().wallet_names();
        for (const auto& [pair, balance] : balances) { // TODO: use one for for actors
            ...
            if (pos == std::string::npos && !pair.second.is_zero()) {
                eInfo("{} {} ROCC", pair.first.to_string(), balance.to_string(NumeralBase::Dec));
            } else {
                eInfo("{} {} ROCC",
...
void ConsoleCommandProcessor::showSectionsCount(const std::string& input) {
    eInfo("Sections count: {}", node.dag()->current_section().to_string(NumeralBase::Dec));
}
...
void ConsoleCommandProcessor::showHelp() {
    ...
    eInfo("  section get <id>                     : Retrieve section information by id");
    eInfo("  dfs add <file_path>                  : Add a file to the Dfs");
    eInfo("  dfs remove <file_id>                 : Remove a file from the Dfs");
    eInfo("  wallet transfer <from> <to> <amount> : Transfer funds between wallets");
...
void ConsoleCommandProcessor::sendDfsAdd(const std::string& input) {
    ...
    const auto& result =
        node.dfs()->store_file(mainId, mainId, file_path, "", file_name, Dfs::DataSecurity::Self, dataSecurity);
    ...
    if (result.has_value()) {
        ...
        eInfo("Failed to add file: {}, error: {}", file_name, getErrorMessage(result.error()));
    }
}

void ConsoleCommandProcessor::sendDfsRemove(const std::string& input) {
    std::vector<std::string> arguments = getExpectedArgument(input, "dfs remove", 1);
    if (arguments.empty()) {
        return;
    }
...
void ConsoleCommandProcessor::sendWalletTransfer(const std::string& input) {
    std::vector<std::string> arguments = getExpectedArgument(input, "wallet transfer", 3);
    if (arguments.empty()) {
        return;
    }
...
void ConsoleCommandProcessor::sendWalletAdd(const std::string& input) {
    ...
    std::vector<std::string> arguments = getExpectedArgument(input, "wallet add", 1);
    if (arguments.empty()) {
...
void ConsoleCommandProcessor::sendWalletRename(const std::string& input) {
    std::vector<std::string> arguments = getExpectedArgument(input, "wallet rename", 2);
    if (arguments.empty()) {
        return;
    }
...
void ConsoleCommandProcessor::exportProfileFile(const std::string& input) {
    std::vector<std::string> arguments = getExpectedArgument(input, "export file", 1);
    if (arguments.empty()) {
        eInfo("Need folder");
        return;
    }
    ...
    QString   fileName = QString("RaccoonLine_%1.profile").arg(now.toString("yyyy_MM_dd_hh_mm_ss"));
...
std::vector<std::string> ConsoleCommandProcessor::getExpectedArgument(const std::string& input,
    ...
    if (words_after_command.size() == expected_arg_count) {
        ...
        eInfo("<{}> expects {} argument(s). {} were provided",
              command,
              expected_arg_count,
              words_after_command.size());
    }
    return {};
}
