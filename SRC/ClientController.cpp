...
QString ClientController::myId() {
    return currentActorId.toQString();
}

void ClientController::connectSignals() {
    // TODO: move to UC
    // registration
    connect(welcomePage.get(), &WelcomePage::logInStarted, this, &ClientController::logIn);
    connect(welcomePage.get(), &WelcomePage::autoLogInStarted, this, &ClientController::autoLogIn);
    ...
    connect(this, &ClientController::authEnded, [this](bool status, int type) {
        ...
        if (file.open(QFile::ReadOnly)) {
            ...
            file.close();
            emit this->sendNotificationToken(QSysInfo::productType(), this->myId(), token);
        }
    });
...
void ClientController::addAvatar(QString imageFile, bool temp, const int x, const int y, const int size) {
    const int avaSize = 200;
    eLog("addingAvatar {} {} {} {} {}", currentActorId.toQByteArray(), QUrl(imageFile).toLocalFile(), x, y, size);
    ...
    avatars.insert(0,
                   QString::fromStdString(
                       Utils::calculate_hash_file(FsPath::create(file.toStdString()).value()).value()));
    currentProfile.setAvatar(avatars);
    QTimer::singleShot(1000, [this]() {
        emit this->updateAvatarImage();
    }); // TODO

    emit sendProfile();
}

bool ClientController::networkStatus() const {
...
ImageResult prepareImage(const QString &fileName, const QString &tmpName) {
    ...
    if (fileSize > 500 && (image.rect().width() > imageSize || image.rect().height() > imageSize)) {
...
bool ClientController::prepareMediaForPostsEvents(QVariantMap &map) {
    ...
    for (const QVariant &file : files) {
        ...
        fileMap["file"]       = QString::fromStdString(
            Utils::calculate_hash_file(FsPath::create(originalImage.toStdString()).value()).value());

        if (type == "video") {
            originalImage   = originalImage.replace(QString::fromStdWString(Utils::filePrefix), "");
            fileMap["file"] = QString::fromStdString(
                Utils::calculate_hash_file(FsPath::create(originalImage.toStdString()).value()).value());
            edited << fileMap;
        ...
        QString randName = QString("tmp/%1").arg(Utils::generate_random_hex(50).c_str());
...
double ClientController::appWidth() const {
    return m_appWidth;
}

int ClientController::networkSockets() const {
...
void ClientController::addEvent(QVariantMap event) {
    ...
    QString eventFile =
        QString::fromStdString(Utils::calculate_hash_file(FsPath::create(tmpName.toStdString()).value()).value());
}
...
QVariantMap ClientController::token(const QString &id) {
    ...
    DbConnector db("chain/tokens.cache");
    auto        rows = db.select("SELECT * FROM Tokens WHERE tokenId = '" + id.toStdString() + "'");
...
bool ClientController::serverError() const {
...
void ClientController::addPortfolio(QString image) {
    ...
    portfolio << QString::fromStdString(
        Utils::calculate_hash_file(FsPath::create(image.toStdString()).value()).value());
...
bool ClientController::isApp() const {
    return m_isApp;
}
...
void ClientController::requestQProfile(const QString &userId, QJSValue callback) {
...
Profile ClientController::profile() const {
    return currentProfile;
}

bool ClientController::needWipe() {
    return m_needWipe;
}

void ClientController::setNeedWipe(bool needWipe) {
    m_needWipe = needWipe;
}
...
void ClientController::logIn() {
    auto result = m_node->login(welcomePage->email().toStdString(), welcomePage->password().toStdString());
...
QVariantList ClientController::multipleProfiles() {
    auto hash = Utils::calculate_hash((welcomePage->email() + welcomePage->password()).toStdString());

    if (!welcomePage->hash().isEmpty() && (welcomePage->email().isEmpty() || welcomePage->password().isEmpty())) {
...
void ClientController::loginPrivateProfile(QByteArray id, QByteArrayList idList) {
...
void ClientController::like(QString userId, QString postId, bool isRemove, bool isPost) {
...
void ClientController::likeComment(QString userId, QString postId, QString commentId, bool isRemove, bool isPost) {
    //    auto contentHash =
    //        Utils::calcKeccak(userId.toLatin1() + postId.toLatin1() +
...
void ClientController::mark(QString userId, QString postId, bool isRemove, bool isPost) {
    //     auto mainActor = m_node->accountController()->mainActor().key();
    //     if (isPost)
...
void ClientController::postComment(QString commentId,
                                   QString userId,
                                   QString postId,
                                   QString text,
                                   QString sub,
                                   bool    isPost) {
    auto contentHash = Utils::calculate_hash(userId.toStdString() + postId.toStdString() + commentId.toStdString()
...
QVariantList ClientController::loadCommentsTemp(QString userId, QString postId, bool isPost) {
    //    QString filePath = CardManager::buildPathForFile(userId.toStdString(),
    //    postId.toStdString() +
    //    ".comments",
    //                                                     isPost ?
...
QVariantList ClientController::loadLikesTemp(QString userId, QString postId, bool isPost) {
    //    QString filePath = CardManager::buildPathForFile(userId.toStdString(),
    //    postId.toStdString() +
    //    ".likes",
    //                                                     isPost ?
...
QString ClientController::myUsername() {
    return usernameById(myId());
}
...
//       SearchEnum::TxParam::Hash, hash.toLatin1(), ActorId(token.toStdString()));
//   std::string data   = transaction.data();
//   bool        incoma = transaction.sender() != wallet->getCurrentWalletId().toStdString();

//   tx["date"]          = transaction.date();
//   tx["amount"]        = QString(transaction.amount().toByteArray());
//   tx["amountDec"]     = QString(transaction.amount().toByteArray(NumeralBase::Dec));
//   tx["amountVisible"] = QString(transaction.amount().toByteArray(NumeralBase::Dec));
//   tx["data"] = QString::fromStdString(data);
//   tx["token"]         = QString(transaction.token().toByteArray());
//   tx["sender"]        = QString(transaction.sender().toByteArray());
//   tx["receiver"]      = QString(transaction.receiver().toByteArray());
//   tx["producer"]      = QString(transaction.producer().toByteArray());
...
//     QElapsedTimer t;
//     t.start();
//     Signature signature = m_node->accountController()->mainActor()->key().sign(hash);

//     QByteArray sign = QByteArray::fromStdString(std::string(signature.begin(), signature.end()));
//     bench["sign"] = bench["sign"].toLongLong() + t.elapsed();

//     t.restart();
//     bool verify = m_node->accountController()->mainActor()->key().verify(hash, signature);
//     Q_UNUSED(verify)
//     bench["verify"] = bench["verify"].toLongLong() + t.elapsed();

//     t.restart();
//     std::string hello = "hello";
//     auto pubKey = m_node->accountController()->mainActor()->key().publicKey();
//     auto encryptedActorBytes = m_node->accountController()->mainActor()
//                               ->key().encrypt(Bytes(hello.begin(), hello.end()), pubKey);
//     auto encryptedActor = std::string(encryptedActorBytes.begin(), encryptedActorBytes.end());
//     bench["encryptActor"] = bench["encryptActor"].toLongLong() + t.elapsed();
//     t.restart();
//     auto decryptedActor =
...
//     KeyPass keyPass = {};
//     std::copy_n(keyd.begin(), std::min(keyd.size(), crypto_box_SEEDBYTES), keyPass.begin());
//     Bytes encryptedFish =Cryptography::encrypt(Bytes(hello.begin(), hello.end()),
//                                                         keyPass);
...
//   //        eLog("{} not exists", "[Chat] " + DfsStruct::ROOT_FOOLDER_NAME + "/" + myId +
//   //        "/private/chats"
//   //);
//   //        return { "", false };
//   //    }
...
int ClientController::getAuthStatus() const {
    return authType;
}

void ClientController::setAuthStatus(int value) {
    authType = value;
}
...
int ClientController::saveFile(QString dfsPath) {
    ...
    QString newFilePath =
        QStandardPaths::writableLocation(QStandardPaths::DownloadLocation) + "/" + QFileInfo(dfsPath).fileName();
...
bool ClientController::uploadFile(QString filePath) {
    ...
    //    eLog("[Upload] Temp copy file result: {} {} {}", copyRes, filePath, //    dfsPath); if (!copyRes) {
    //        return false;
    //    }
    //    QString fileName = QFileInfo(filePath).fileName();
    // #ifdef Q_OS_ANDROID
...
//         QUrl(QString::fromStdWString(Utils::filePrefix) + decryptedFile, QUrl::TolerantMode));
// #else
//     if (openType == "view")
...
QString ClientController::calcRoccSummary() {
    ...
    //     return QString::fromStdString(fmt::format("Error: {}", mega_result.error()));
    // }

    // BigNumberFloat sum;

    // for (const auto &[first, second] : mega_result->dataRows()) {
    //     if (first.tokenId == ActorId("468faf2f1be6504a9a26f7f027f7e43380b0d77d")) {
    //         sum += second.state;
    //     }
    // }
...
void ClientController::installUpdate() {
...
QVariantList ClientController::loadUserNames() {
    ...
    if (usernames_file_id.empty()) {
        auto search_result = Dfs::Tables::ActorDirFile::search_file_by_folder_and_name(network_id,
...
bool ClientController::addUsername(QString username) {
    ...
    auto res = m_node->dfs()->add_vector_row(network_id,
                                             usernames_file_id,
                                             {
...
bool ClientController::existsUsername(QString username) {
    ...
    auto row = v->read_rows(fmt::format("WHERE name='{}' AND status = '1'", username.toStdString()));
    if (!row.has_value()) {
        return false;
    }
    if (row->empty()) {
...
void ClientController::loadSubscription() {
    ...
    if (sub_file_id.empty()) {
        auto search_result = Dfs::Tables::ActorDirFile::search_file_by_folder_and_name(raccoon_id,
                                                                                       Dfs::Basic::TEMPLATE_VECTOR,
                                                                                       "RaccoonSubscription");
        if (!search_result.has_value()) {
            return;
...
void ClientController::addSubscription(int type, bool auto_renew) {
    ...
    bool res = m_node->add_subscription(raccoon_id,
                                        sub_file_id,
                                        type,
                                        auto_renew,
...
//     QObject::connect(downloader, &FileDownloader::downloadProgress, [](qint64 received, qint64 total) {
...
//     QObject::connect(downloader, &FileDownloader::error, [](const QString &error) {
//         qDebug() << "Error:" << error;
//     });

//     downloader->downloadFile("https://raccoonline.com/api/assets/apps/RaccoonLine_Setup_0.2.0.dmg",
//                              "./RaccoonLine_Setup_0.2.0.dmg");
...
void ClientController::setNode(ExtraChainNode *newNode) {
    ...
    connect(m_node->dfs(), &DfsController::vectorRowRemoved, setupUsernamesHandler);
}
...
QString ClientController::testButton(const QString &f) {
    return "";
}
...
void ClientController::addFiles(const QStringList &files,
    ...
    auto processFile = [this, file, mainId, dataSecurity, service_folder, messenger_send_as_file]() {
        try {
            ...
            auto result = m_node->dfs()->store_file(mainId,
                                                    mainId,
            ...
            if (!result.has_value()) {
                ...
                case Dfs::DfsError::MaxFileSize:
                    ...
                    break;
                default:
                    message = QString::fromStdString(fmt::format("Error: {}", result.error()));
                    break;
                }
            } else {
                message = QFileInfo(file).fileName() + " was successfully uploaded to storage.";
            }

            if (result.has_value() && sf == Dfs::ServiceFolder::Chat) {
                const auto   &value   = result.value();
                const QString dfsPath = QString::fromStdWString(
                    DfsB::fsActrRootW + DfsB::separator + value.actor_id.toQString().toStdWString()
                    + DfsB::separator + QString::fromStdString(value.file_id).toStdWString());
                emit messengerAddedFile(dfsPath, file, messenger_send_as_file);
            } else if (!result.has_value() && result.error() == Dfs::DfsError::DirDuplicate
                       && sf == Dfs::ServiceFolder::Chat) {
                const std::string filename = QFileInfo(file).fileName().toStdString();
                const auto        searchResult =
                    Dfs::Tables::ActorDirFile::search_file_by_folder_and_name(mainId,
                                                                              Dfs::Basic::TEMPLATE_CHAT,
                                                                              filename);
                const auto   &fileId  = searchResult.value().file_id;
                const QString dfsPath = QString::fromStdWString(
                    DfsB::fsActrRootW + DfsB::separator + QString::fromStdString(mainId.to_string()).toStdWString()
...
bool ClientController::subscribed() const {
    return m_subscribed;
}

bool ClientController::subscribeActive() const {
    return m_subscribeActive;
}

bool ClientController::usernameActive() const {
    return m_usernameActive;
}
