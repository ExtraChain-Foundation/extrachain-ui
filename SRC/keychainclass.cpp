...
KeyChainClass::KeyChainClass(QObject *parent)
    : QObject(parent)
      ...
      ,
      m_readCredentialJob(QLatin1String("com.extrachain.messenger")),
      m_writeCredentialJob(QLatin1String("com.extrachain.messenger")),
      m_deleteCredentialJob(QLatin1String("com.extrachain.messenger")) {
#else
      ,
      m_readCredentialJob(QLatin1String("com.extrachain.app")),
      m_writeCredentialJob(QLatin1String("com.extrachain.app")),
      m_deleteCredentialJob(QLatin1String("com.extrachain.app")) {
#endif
...
void KeyChainClass::readKey(const QString &key) {
  ...
  QObject::connect(
      &m_readCredentialJob, &QKeychain::ReadPasswordJob::finished, this,
      [this, key]() {
...
void KeyChainClass::writeKey(const QString &key, const QString &value) {
  ...
  QObject::connect(
      &m_writeCredentialJob, &QKeychain::WritePasswordJob::finished, this,
      [this, key]() {
...
void KeyChainClass::deleteKey(const QString &key) {
  ...
  QObject::connect(
      &m_deleteCredentialJob, &QKeychain::DeletePasswordJob::finished, this,
      [this, key]() {
...
void KeyChainClass::readKeyAsUser(const QString &key) {
  ...
  uid_t sudo_uid =
      getenv("SUDO_UID") ? QString(getenv("SUDO_UID")).toInt() : 1000;
  gid_t sudo_gid =
      getenv("SUDO_GID") ? QString(getenv("SUDO_GID")).toInt() : 1000;
  ...
  QKeychain::ReadPasswordJob *tempJob =
      new QKeychain::ReadPasswordJob(m_readCredentialJob.service());
  tempJob->setKey(key);

  QObject::connect(tempJob, &QKeychain::ReadPasswordJob::finished, this,
                   [this, tempJob, key, old_euid, old_egid]() {
                     ...
                     if (tempJob->error()) {
                       emit error(tr("Read key failed: %1")
                                      .arg(qPrintable(tempJob->errorString())));
                     } else {
...
void KeyChainClass::writeKeyAsUser(const QString &key, const QString &value) {
  ...
  // Create temporary job for keychain operation
  QKeychain::WritePasswordJob *tempJob =
      new QKeychain::WritePasswordJob(m_writeCredentialJob.service());
  tempJob->setKey(key);
  tempJob->setTextData(value);

  QObject::connect(tempJob, &QKeychain::WritePasswordJob::finished, this,
                   [this, tempJob, key, old_euid, old_egid]() {
                     // Restore root privileges
                     seteuid(old_euid);
                     setegid(old_egid);
...
void KeyChainClass::deleteKeyAsUser(const QString &key) {
  ...
  // Create temporary job for keychain operation
  QKeychain::DeletePasswordJob *tempJob =
      new QKeychain::DeletePasswordJob(m_deleteCredentialJob.service());
  tempJob->setKey(key);

  QObject::connect(tempJob, &QKeychain::DeletePasswordJob::finished, this,
                   [this, tempJob, key, old_euid, old_egid]() {
                     // Restore root privileges
                     seteuid(old_euid);
                     setegid(old_egid);
