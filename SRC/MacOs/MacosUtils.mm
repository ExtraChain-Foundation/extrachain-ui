...
#import <UIKit/UIKit.h>
    ...
    !defined(EXTRACHAIN_RELEASE_WITH_DEBUG_INFO)
...
void ApplePlatformUtils::clipboard(const QString &text) {
  ...
  //
  //    NSString *nsText = [NSString
  //    stringWithUTF8String:text.toUtf8().constData()]; NSArray *objectsToPaste
  //    = @[ nsText ]; [pasteboard writeObjects:objectsToPaste];
}
...
- (NSString *)feedURLStringForUpdater:(SPUUpdater *)updater {
  return @"https://extrachain.com/api/assets/apps/appcast_rl_vpn.xml";
}
...
- (NSTimeInterval)
    standardUserDriverRemindMeLaterInterval:(SPUStandardUserDriver *)userDriver
...
#ifdef Q_OS_IOS
void ApplePlatformUtils::authenticateWithFaceID(
...
void ApplePlatformUtils::triggerFaceID() {
  ...
  authenticateWithFaceID([safeSelf](bool success, const char *errorMessage) {
    qDebug() << "Inside callback, success:" << success
             << " error:" << errorMessage;
