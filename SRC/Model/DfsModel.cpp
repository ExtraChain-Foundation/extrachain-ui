...
DfsModel::DfsModel(ExtraChainNode &node, VariantModel *parent)
  ...
  QObject::connect(node.dfs(), &DfsController::downloaded,
                   [this](ActorId owner_id, Dfs::DirRow dirRow) {
                     this->updateFileProgress(dirRow.actor_id, dirRow.file_id,
                                              100);
                   });
  QObject::connect(node.dfs(), &DfsController::uploadProgress, this,
                   &DfsModel::updateFileProgress);
  ...
  //                     {
  //                         eInfo("[From/DFS] Upload progress: {} {}   {}",
  //                         actorId
...
void DfsModel::jumpToFolder(const QString &folder) {
  ...
  auto dirRows = Dfs::Tables::DirsFile::ActorSpace::get_dir_rows(
      node.dfs()->get_db_instance(), currentDirActor);
  ...
  for (auto &dirRow : dirRows.value()) {
    ...
    try {
      int progress = std::filesystem::file_size(Dfs::Path::filePath(
                         currentDirActor, dirRow.file_id)) == dirRow.size
                         ? 100
                         : 0;
      append(toMap(DfsModelDirType::File, dirRow.visual_path(), dirRow.file_id,
                   dirRow.size, progress));
    } catch (...) {
...
void DfsModel::removeFile(int index) {
  ...
  auto path =
      QString::fromStdString(
          fmt::format("{}/{}", QDir::currentPath().toStdString(),
                      Dfs::Path::filePath(ActorId(currentDir), hash).string()))
          .replace("\\", "/");
...
void DfsModel::addFile(ActorId owner_id, Dfs::DirRow dirRow) {
  ...
  try {
    progress = std::filesystem::file_size(Dfs::Path::filePath(
                   dirRow.actor_id, dirRow.hash)) == dirRow.size
                   ? 100
                   : 0;
  } catch (std::exception &) {
  }
...
void DfsModel::updateFileProgress(ActorId actorId, std::string fileName,
                                  int progress) {
...
QVariantMap DfsModel::toMap(DfsModelDirType isDir, std::string name,
                            std::string hash, qint64 size, int progress) {
...
const QString &DfsModel::currentActor() const { return m_currentActor; }
