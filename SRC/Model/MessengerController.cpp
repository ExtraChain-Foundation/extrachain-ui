...
UserModel::UserModel(VariantModel *parent)
    : VariantModel(parent) {
...
ChatListModel::ChatListModel(VariantModel *parent)
    : VariantModel(parent) {
    setModelRoles({ "name_chat",
                    "myself",
                    "another",
                    "type",
                    "fileActorId",
...
ChatModel::ChatModel(VariantModel *parent)
    : VariantModel(parent) {
...
MessengerController::MessengerController(ExtraChainNode *nde, QObject *parent)
    ...
    connect(node, &ExtraChainNode::chatsLoaded, [this]() {
    ...
    connect(node, &ExtraChainNode::messageAdded, [this](ActorId owner_id, std::string file_id, Chat::Message msg) {
    ...
    connect(node, &ExtraChainNode::messageRemoved, [this](ActorId owner_id, std::string file_id, std::string id) {
        if (owner_id == this->owner_id && file_id == this->file_id) {
...
UserModel *MessengerController::userModel() const {
    return _userModel;
}
...
ChatModel *MessengerController::chatModel() const {
    return _chatModel;
}
...
QVariantMap MessengerController::chatToMap(const Chat::Chat &chat) {
    ...
    if (chat.chat.chat_type.has_value() && chat.chat.chat_type.value() == Chat::ChatType::Channel) {
        name = "RaccoonLine News";
    ...
    if (messages.has_value() && !messages.value().empty()) {
        ...
        dt_last_message           = qulonglong(messages.value().at(messages.value().size() - 1).timestamp);
    ...
    return { { "myself", node->account_controller()->current_profile().main_id().toQString() },
             ...
             { "name_chat", name },
             { "type", chat.chat.chat_type.has_value() ? int(chat.chat.chat_type.value()) : 0 },
...
void MessengerController::updateChatListModel() {
    ...
    Chat::Chat channel = Chat::Chat {
        ...
        .file_id  = "19dcb46ef1a9c982dfd56cd9f1e823290f9a8bbadcdad09679563dfba5a96a7c",
...
void MessengerController::updateChatModel(QString fileOwnerId, QString fileId, bool quick) {
...
void MessengerController::addMessage(const Chat::Message &message) {
    ...
    mapMessage.insert("text", QString::fromStdString(message.message.data.value_or("")));
    if (message.message.data.has_value()) {

        QJsonParseError parseError;
        QJsonDocument   doc =
            QJsonDocument::fromJson(QString::fromStdString(message.message.data.value().c_str()).toLatin1(),
                                    &parseError);
        ...
        if (message.message.type.has_value() && message.message.type.value() == Chat::MessageType::Video
            && !path.isEmpty()) {
            mapMessage["text"] = path;
        }
        if (message.message.type.has_value() && message.message.type.value() == Chat::MessageType::File
            && !path.isEmpty()) {
            mapMessage["text"] = path;
        }
    }
    ...
    if (message.message.reply_id.has_value()) {
        const QString parentMessageId = QString::fromStdString(message.message.reply_id.value());
        ...
        for (int i = 0; i < _chatModel->rowCount(); ++i) {
            ...
            if (rowData.value("messageId").toString() == parentMessageId) {
                ...
                mapMessage.insert("reply_text_message", parentMessage.value("text").toString());
...
void MessengerController::addMessages(const std::vector<Chat::Message> &messages) {
...
void MessengerController::sendMessage(QString text) {
    // static const QRegularExpression controlCharsRegex("[\\x00-\\x08\\x0B\\x0C\\x0E-\\x1F\\x7F]");
    // static const QRegularExpression zeroWidthCharsRegex("[\\u200B-\\u200F\\u2060-\\u2064\\uFEFF]");
    ...
    if (text.startsWith("dfs/") && text.split(QRegularExpression(R"([\\/])")).size() == 3) {
...
void MessengerController::replyMessage(const QString &text, const QString &messageId) {
    qDebug() << "Begin reply to message with id " << messageId << " with text " << text;
    auto message = Chat::MessageText { .text = text.toStdString(), .reply_id = messageId.toStdString() };
    node->chat_manager()->add_new_message_text(owner_id, file_id, message);
}

void MessengerController::sendGif(const QString &gifUrl, const QString &messageId) {
...
void MessengerController::sendImage(const QString &pathToFile,
    ...
    auto res = node->chat_manager()->add_image_message(owner_id, file_id, chatMessageData);
}

void MessengerController::sendVideo(const QString &pathToFile, const QString &messageId, const QString &caption) {
    QMap<QString, QVariant> map;
    map["path"] = pathToFile;
    if (!caption.isEmpty())
        map["caption"] = caption;
    ...
    if (!messageId.isEmpty())
        chatMessageData.reply_id = messageId.toStdString();
    auto res = node->chat_manager()->add_video_message(owner_id, file_id, chatMessageData);
}

void MessengerController::sendFile(const QString &pathToFile, const QString &messageId, const QString &caption) {
    QMap<QString, QVariant> map;
    map["path"] = pathToFile;
    if (!caption.isEmpty())
        map["caption"] = caption;
    ...
    auto res = node->chat_manager()->add_file_message(owner_id, file_id, chatMessageData);
}

void MessengerController::editMessage(const QString &text, const QString &messageId) {
    qDebug() << QString("Edit message with id %1. Text: message: [%2]").arg(messageId, text);
}
...
bool MessengerController::createChat(QString actorWith) {
    ...
    auto my_chats = chat_manager->get_my_chats();
    if (my_chats.has_value()) {
        my_chats_file_id = my_chats->file_id;
    }
...
QString MessengerController::currentDir() {
...
QString MessengerController::getFileNameFromMessage(const QString &message) {
    ...
    if (fileParts.size() != 3) {
        eLog("[MessengerController::getFileNameFromMessage] Please try again or check your export path.", message);
        return QString();
    }
    ActorId actorId(fileParts.at(1).toStdString());
    auto    fileId = fileParts.at(2).toStdString();

    auto dir_row_result = Dfs::Tables::ActorDirFile::get_dir_row(actorId, fileId);
    if (!dir_row_result.has_value())
        return QString();

    QString    name_file = QString::fromStdString(dir_row_result->name);
    const auto chat =
        node->chat_manager()->get_chat(ActorId(_current_owner_id.toStdString()), _current_file_id.toStdString());
    if (!chat) {
    ...
    const auto    decrypted     = Cryptography::symmetric_decrypt(ByteArray(encryptedName.value()).toBytes(),
                                                           Dfs::DataSecurityKey { .key = chat->chat_key }.key);
    const QString nameFile =
        QString::fromUtf8(reinterpret_cast<const char *>(decrypted->data()), static_cast<int>(decrypted->size()));
    return nameFile;
}

void MessengerController::exportFile(const QString &message, const QString &export_path) {
    auto fileParts = message.split(QRegularExpression(R"([\\/])"));
    if (fileParts.size() != 3) {
        emit error("Export failed. Please try again or check your export path.");
        eLog("Export failed. Please try again or check your export path.", message, export_path);
        return;
    }
    ...
    QString exportPath = QString::fromStdString(Dfs::Path::filePath(actorId, fileId).string()).replace("\\", "/");

    auto fs_path = FsPath::create(export_path.toStdString());
    if (!fs_path.has_value())
        return;

    auto isExported = export_file(actorId,
                                  fileId,
                                  fs_path.value(),
                                  nameFile.toStdString(),
                                  Dfs::DataSecurityKey { .key = chat->chat_key });

    if (isExported.has_value()) {
        emit exported(QString("The file has been exported to: %1").arg(export_path));
    } else {
        emit error("Failed to export the file.");
    }
}

QString MessengerController::storeImageToCache(const QString &messageData, const bool &isBlurState) {
    QMap<QString, QVariant> map;

    QJsonParseError parseError;
    QJsonDocument   doc = QJsonDocument::fromJson(messageData.toUtf8(), &parseError);
    ...
    const QString relativePath =
        QString::fromStdString(Dfs::Path::filePath(actorId, fileId.toStdString()).string()).replace("\\", "/");

    const QString fullPath = QDir::currentPath() + "/" + relativePath;
    const QString fileUrl  = QUrl::fromLocalFile(fullPath).toString();

    const auto dirRow = Dfs::Tables::ActorDirFile::get_dir_row(actorId, fileId.toStdString());
    if (!dirRow) {
    ...
    const QString tempFilePath    = QString("tmp/files/dfs/%1/%2").arg(actorId.toQString(), nameFile);
    ...
    if (unsupported || nameFile == "."
        || !QStringList { "png", "jpg", "jpeg", "gif" }.contains(extension, Qt::CaseInsensitive)) {
        eLog("[MessengerController::storeImageToCache] {}", fmt::format("Unsupported extension: {}", extension));
        return QString();
    }

    const QString exportDir = QDir::currentPath() + "/tmp/files/dfs/" + actorId.toQString();
    QDir          dir;
    ...
    if (!fsPath) {
        eLog("[MessengerController::storeImageToCache] {}", fmt::format("Invalid create FsPath"));

        return QString();
    }

    const auto exportRes = export_file(actorId,
                                       fileId.toStdString(),
                                       fsPath.value(),
                                       nameFile.toStdString(),
                                       Dfs::DataSecurityKey { .key = chat->chat_key });
    if (exportRes) {
        eLog("[MessengerController::storeImageToCache] {}",
             fmt::format("Export successful: {}", QUrl::fromLocalFile(absTempFilePath).toString()));
        return QUrl::fromLocalFile(absTempFilePath).toString();
    }

    qDebug() << (int)exportRes.error() << actorId.toQString() << fileId.toStdString()
             << fsPath.value().filename().value() << absTempFilePath;
...
void MessengerController::updateCacheInput(const QString &cahtId, const QString &message) {
...
void MessengerController::addFiles(const QStringList &files,
    ...
    auto processFile = [this, file, mainId, dataSecurity, &messenger_send_as_file, &replyMessageId, &caption]() {
        try {
            ...
            auto result = node->dfs()->store_file(mainId,
                                                  mainId,
                                                  filePath.toStdWString(),
                                                  Dfs::ServiceFolder::Chat,
            ...
            QString message = result.has_value()
                                  ? "File added"
                                  : QString::fromStdString(fmt::format("Add Files Error: {}", result.error()));
            ...
            if (result.has_value()) {
                ...
                emit messengerAddedFile(dfsPath, file, messenger_send_as_file, replyMessageId, caption);
            }
...
std::expected<void, ExportFileError> MessengerController::export_file(const ActorId              &owner_id,
    ...
    if (dir_row_result->encryption) {
        ...
        auto decrypt_result = Cryptography::symmetric_decrypt_file(dfs_path, output_path, dsk.key);
...
QString BlurHashEncoder::encode(const QImage &image, int componentsX, int componentsY) {
    QImage img    = image.convertToFormat(QImage::Format_RGB32);
    int    width  = img.width();
        ...
        for (int x = 0; x < componentsX; ++x) {
            ...
            for (int j = 0; j < height; ++j) {
                for (int i = 0; i < width; ++i) {
                    float basis = qCos(M_PI * x * i / width) * qCos(M_PI * y * j / height);
    ...
    for (int i = 1; i < factors.size(); ++i) {
        ...
        maxAc = std::max(maxAc, std::max(std::abs(f.x()), std::max(std::abs(f.y()), std::abs(f.z()))));
    }
    ...
    for (int i = 1; i < factors.size(); ++i) {
        ...
        auto quant = [](float val) {
            return qBound(-9, int(qRound(val * 9)), 9);
        };
