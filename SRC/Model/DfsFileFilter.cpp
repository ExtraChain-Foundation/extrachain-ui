...
DfsFileFilterModel::DfsFileFilterModel(ExtraChainNode *node, bool filterDir,
                                       VariantModel *parent)
    : VariantModel(parent), node(node), _filterDir(filterDir),
      _maxSpace(max_space_in_gb) {
  setModelRoles({"isDir", "name", "fileId", "size", "progress", "path", "type",
                 "base64", "encrypted", "created"});
  showMain();

  if (node->dfs()) {
    QObject::connect(node->dfs(), &DfsController::added, this,
                     &DfsFileFilterModel::addFile);
    QObject::connect(node->dfs(), &DfsController::uploaded,
                     [this](ActorId owner_id, Dfs::DirRow dirRow) {
                       this->updateFileProgress(dirRow.actor_id, dirRow.file_id,
                                                100);
                     });
    QObject::connect(
        node->dfs(), &DfsController::downloaded,
        [this, &node](ActorId owner_id, Dfs::DirRow dirRow) {
          ...
          auto pathtoFile =
              fmt::format("dfs/{}/{}", dirRow.actor_id, dirRow.file_id);
        });
    QObject::connect(node->dfs(), &DfsController::uploadProgress, this,
                     &DfsFileFilterModel::updateFileProgress);
    QObject::connect(node->dfs(), &DfsController::downloadProgress, this,
                     &DfsFileFilterModel::updateFileProgress);
  }
}
...
void DfsFileFilterModel::jumpToFolder(const QString &folder) {
  ...
  if (!_filterDir) {
    for (auto &dirRow : dirRows.value()) {
      ...
      if (dirRow.folder.has_value() && !dirRow.folder->empty() &&
          dirRow.folder->at(0) == ':') {
        continue;
      }
      ...
      int progress;
      try {
        progress = std::filesystem::file_size(Dfs::Path::filePath(
                       ActorId(currentDir), dirRow.file_id)) == dirRow.size
                       ? 100
                       : 0;
      ...
      if (dirRow.encryption) {
        auto actor = node->account_controller()->current_profile().get_actor(
            ActorId(currentDir));
        auto encrypted_name = Utils::from_base64(name);
        if (actor.has_value() && encrypted_name.has_value()) {
          auto res = actor->get().key().decrypt_self(
              ByteArray(encrypted_name.value()).toBytes());
          if (res.has_value()) {
      ...
      append(toMap(DfsModelDirType::File, name, dirRow.file_id, dirRow.size,
                   progress, dirRow.encryption));
    }
  }
...
void DfsFileFilterModel::openFile(const int &index) {
  ...
  auto path = QString::fromStdString(
                  Dfs::Path::filePath(ActorId(currentDir), fileId).string())
                  .replace("\\", "/");
  auto tempPath = "tmp/files/" + name;
  auto fullTempPath = QDir::currentPath() + "/" + tempPath;

  if (!QFile::exists(tempPath) &&
      QFile(path).size() != QFile(tempPath).size()) {
...
void DfsFileFilterModel::exportFile(const int &index,
  ...
  QString path = QString::fromStdString(
                     Dfs::Path::filePath(ActorId(currentDir), fileId).string())
                     .replace("\\", "/");
  QString tempPath = QString("tmp/files/%1/%2")
                         .arg(QString::fromStdString(currentDir))
                         .arg(name);
  ...
  emit message(
      copied ? "File exported"
             : fmt::format("Failed export file: {}", exported.error()).c_str());
  eLog("Export file to folder: {}. Export path: {}. Result: {}", fullTempPath,
       exportPathFile, copied ? "saved" : "not saved");
}
...
QString DfsFileFilterModel::exportData(const int &index) {
  ...
  QString tempPath =
      QString("tmp/files/dfs/%1").arg(QString::fromStdString(currentDir));
  QString pathToFileInTmpFolder = QDir::currentPath() + "/" +
                                  QDir(tempPath).path() + "/" +
                                  map["name"].toString();
...
void DfsFileFilterModel::addFile(ActorId owner_id, Dfs::DirRow dirRow) {
  ...
  emit fileAdded();
  int progress = std::filesystem::file_size(Dfs::Path::filePath(
                     dirRow.actor_id, dirRow.file_id)) == dirRow.size
                     ? 100
                     : 0;
  calcUsedSpace();
...
void DfsFileFilterModel::updateFileProgress(ActorId actorId,
                                            std::string fileName,
                                            int progress) {
...
void DfsFileFilterModel::searchIn(const QString &text) {
  ...
  auto isValidFileExtensionPattern = [](const QString &pattern) -> bool {
    return QRegularExpression(R"(^\*\.[a-zA-Z0-9]+$)")
        .match(pattern)
        .hasMatch();
  };
  ...
  std::ranges::copy_if(_cacheList, std::back_inserter(founded),
                       [&](const QVariantMap &item) {
                         QString nameFile = item["name"].toString().toLower();
                         eLog("{}", item);

                         return isByExtension ? nameFile.endsWith(ext)
                                              : nameFile.contains(searchText);
                       });
...
QVariantMap DfsFileFilterModel::toMap(DfsModelDirType isDir, std::string name,
                                      std::string file_id, qint64 size,
                                      int progress, bool encrypted,
                                      bool isImage) {
  auto path = QDir::currentPath() + "/" +
              QString::fromStdString(
                  Dfs::Path::filePath(ActorId(currentDir), name).string())
                  .replace("\\", "/");
  ...
  QFileInfo fi(QDir::currentPath() + "/" +
               QString::fromStdString(
                   Dfs::Path::filePath(ActorId(currentDir), file_id).string())
                   .replace("\\", "/"));
...
QString DfsFileFilterModel::toBase64(const std::string &fileId,
  ...
  QString tempPath =
      QString("tmp/files/dfs/%1").arg(QString::fromStdString(currentDir));
  auto actor_folder = DfsB::DFS_FOLDER + Utils::platformDelimeter() +
                      ActorId(currentDir).to_string();
  QString pathToTmpExportDir =
      QDir::currentPath() + "/" + QDir(tempPath).path();
    ...
    if (f.exists()) {
      if (f.open(QIODevice::ReadOnly)) {
        ...
        return QString("data:%1;base64,%2")
            .arg(mimeType, QString::fromLatin1(data.toBase64()));
      } else {
...
void DfsFileFilterModel::calcUsedSpace() {
  QString folderPath = QString::fromStdString(
      Dfs::Path::actorPath(ActorId(currentDir)).string());
...
int DfsFileFilterModel::maxSpace() const { return _maxSpace; }
...
double DfsFileFilterModel::usedSpace() const { return _usedSpace; }
