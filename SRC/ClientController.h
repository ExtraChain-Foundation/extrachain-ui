...
class ClientController : public QObject {
  Q_OBJECT
  Q_PROPERTY(bool networkStatus READ networkStatus WRITE setNetworkStatus NOTIFY
                 networkStatusChanged)
  Q_PROPERTY(int networkSockets READ networkSockets WRITE setNetworkSockets
                 NOTIFY networkSocketsChanged)
  Q_PROPERTY(bool serverError READ serverError WRITE setServerError NOTIFY
                 serverErrorChanged)
  Q_PROPERTY(
      Profile profile READ profile WRITE setProfile NOTIFY profileChanged)
  Q_PROPERTY(
      double appWidth READ appWidth WRITE setAppWidth NOTIFY appWidthChanged)
  Q_PROPERTY(bool isApp READ isApp WRITE setIsApp NOTIFY isAppChanged)
  Q_PROPERTY(
      QString authHash READ authHash WRITE setAuthHash NOTIFY authHashChanged)

  Q_PROPERTY(bool subscribed READ subscribed NOTIFY subscribedChanged)
  Q_PROPERTY(
      bool subscribeActive READ subscribeActive NOTIFY subscribeActiveChanged)
  Q_PROPERTY(
      bool usernameActive READ usernameActive NOTIFY usernameActiveChanged)
...
public:
  ...
  Q_INVOKABLE bool importPhrase(const QString &login, const QString &password,
                                const QString &phrase);
  Q_INVOKABLE bool importHex(const QString &login, const QString &password,
                             const QString &hex);
...
public slots:
  ...
  void addAvatar(QString imageFile, bool temp, const int x, const int y,
                 const int size);
  ...
  void likeComment(QString userId, QString postId, QString commentId,
                   bool isRemove, bool isPost);
  void mark(QString userId, QString postId, bool isRemove, bool isPost);
  void postComment(QString commentId, QString userId, QString postId,
                   QString text, QString sub, bool isPost);
  ...
  QVariantList loadCommentsLikesTemp(QString userId, QString postId,
                                     QString commentId, bool isPost);
  ...
  // int exportFile(QString dfsPath, QString origName, int originSize, QString
  // chatId);
  ...
  // void openChatFile(QString chatId, QString dfsPath, QString originName, int
  // originSize, QString openType);
  void skipAuth();
  ...
  void addFiles(const QStringList &files, const int service_folder = 0,
                const bool messenger_send_as_file = false);
  QString fileName(const QString &file);

signals:
  void prepare();
  void connectionError(Network::SocketServiceError error, QString identifier,
                       QString errorData);
  void generateSmartContract(QByteArray tokenCount, QByteArray tokenName,
                             QByteArray rulAddress, QByteArray color);
  void connectToNode(const QString &ip, Network::Protocol protocol,
                     bool request = true, bool isConstant = true,
                     bool isLight = false);
  ...
  void messengerAddedFile(const QString &pathToFile,
                          const QString &selectedFile, const bool &asFile);
